...
def generate_lambda_pairs(l_min: int, l_max: int) -> List[Tuple[int, int]]:
    """Генерирует пары корней (lambda_1, lambda_2) характеристического уравнения, из здаанного диапазона,
...
def calculate_abc_parameters(lambda_pairs: List[Tuple[int, int]]) -> List[Tuple[int, int, int, int]]:
    """ Вычисляет параметры a, b, c из корней lamda_1, lambda_2, по теореме Виета
...
def generate_bc_factorizations(parameters: List[Tuple[int, int, int, int]], not_grate_than=15) -> List[Tuple[int, int, int, int, int]]:
    ...
    Returns:
        List[Tuple[int, int, int, int, int]]: Список кортежей парамтеров и лямбд: a, b, c, l1, l2
    """
    all_paramters = []
    ...
    for a, bc, l1, l2 in parameters:
        ...
        for b in divisors:
            c = bc // b
            if abs(b) <= not_grate_than and abs(c) <= not_grate_than and b != 0 and c != 0:
                all_paramters.append((a, b, c, l1, l2))
                all_paramters.append((a, -b, -c, l1, l2))
                    
    return all_paramters


def calculate_constants(all_parameters: List[Tuple[int, int, int, int, int]]) -> List[Dict]:
    """Вычсиляет значение констант для общего решения

    Args:
        all_parameters (List[Tuple[int, int, int, int, int]]): Список кортежей парамтеров и лямбд: a, b, c, l1, l2
...
def is_nice_fractions(rational_number: float, tol: float = 1e-10) -> bool:
    """Проверяет, явялется ли константа хорошей дробью

    Args:
        rational_number (float): число для првоерки
        tol (float, optional): для првоерки на одиноковсть разности чисел с плаващий точкой. Defaults to 1e-10.
